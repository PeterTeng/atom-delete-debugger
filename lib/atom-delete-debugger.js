...
export default {
  ...
  async deleteDebugger() {
    const statements = atom.config.get('atom-delete-debugger.debuggerStatements').join(', ');
    const message = `This will delete ${statements} from this project`;
    ...
    const statementsRegex = atom.config.get('atom-delete-debugger.debuggerStatements').map(statement => {
      statement += ';?';
      return statement;
    }).join('|');

    const flags = 'g';
    const regex = RegExp(statementsRegex, flags);
