...
export default {
  ...
  async deleteDebugger() {
    const expressions = atom.config.get('atom-delete-debugger.debuggerExpressions').join(', ');
    const message = `This will delete ${expressions} from this project`;
    const buttonChosen = atom.confirm({
      message: 'Are you sure you want to delete all debugger expressions?',
      detailedMessage: message,
      buttons: ['OK', 'Cancel']
    });

    const expressionsRegex = atom.config.get('atom-delete-debugger.debuggerExpressions').map(expression => {
      expression += ';?';
      return expression;
    }).join('|');

    const flags = 'g';
    const regex = RegExp(expressionsRegex, flags);
    const filePaths = await this.debuggerPaths(regex);

    if (buttonChosen === 0) {
    return this.replace(regex, '', filePaths);
    }
    return;
  }
